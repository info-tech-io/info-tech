...
* **`infotecha`** - Центральный хаб, запускающий сборку модулей
...
2. **Децентрализованная модульная структу��а**: Каждый образовательный модуль - отдельный репозиторий с `module.json`
3. **Гибкая система шаблонов**: `hugo-templates` обеспечивает кастомизацию внешнего вида и функциональности
4. **Автоматизированная сборка**: CI/CD процесс управляется метаданными из `module.json`
...
1. Контент и/или `module.json` обновляются в репозитории `mod_*`
2. GitHub webhook уведомляет центральный хаб (`infotecha`)
3. GitHub Actions workflow в `infotecha`:
   - Читает `module.json` из репозитория модуля
   - Клонирует `hugo-templates`
   - Запускает скрипт сборки из `hugo-templates` с параметрами из `module.json`
...
- Жесткого единого шаблона (`hugo-base`)
- Централизованного реестра `modules.json`

**Реализовали:**
- Гибкую фабрику шаблонов `hugo-templates`
- Децентрализованную конфигурацию через `module.json`
...
- Единообразный UX/UI через `hugo-templates`
